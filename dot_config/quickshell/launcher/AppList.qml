...
Item {
    ...
    Process {
        ...
        command: ["bash", "-lc",
            ...
            // Highest-priority dirs first: a user override with the same
            // filename (desktop id) wins, so an app isn't listed twice even
            // when the overriding Exec differs (XDG desktop-id semantics).
            "for f in " +
            ...
            "  [ -z \"$exec\" ] && continue; " +
            "  printf '%s\\t%s|%s\\n' \"$id\" \"$name\" \"$exec\"; " +
            "done | awk -F'\\t' '!seen[$1]++ {print $2}' | sort -u"
        ]
        running: true
        stdout: SplitParser {
            property var apps: []
