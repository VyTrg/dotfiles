...
Item {
    ...
    // The fcitx5 input-method item is shown by the dedicated KeyboardLayout
    // widget instead, so hide it from the tray to avoid a redundant indicator.
    function _isInputMethod(item) {
        const text = _traySearchText(item)
        return _hasAny(text, ["fcitx", "unikey", "input method", "input-keyboard"])
    }
    ...
    Row {
        ...
        Repeater {
            ...
            Item {
                ...
                height: 18
                anchors.verticalCenter: parent.verticalCenter
                // Hidden items are skipped by Row without leaving a gap.
                visible: !root._isInputMethod(modelData)
