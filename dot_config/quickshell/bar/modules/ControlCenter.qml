...
PanelWindow {
    ...
    // Stay mapped while the close animation plays out; the panel drives the
    // fade/scale and this keeps the surface alive until it finishes.
    visible: showing || panel.opacity > 0.01
    ...
    // Power profile (via net.hadess.PowerProfiles D-Bus, provided by
    // power-profiles-daemon or tuned-ppd). Empty string => unavailable.
    property string powerProfile:   ""
    readonly property string ppLabel: powerProfile === "performance" ? "Performance"
        : powerProfile === "power-saver" ? "Battery Saver" : "Balanced"
    readonly property string ppIcon: powerProfile === "performance" ? "󰓅"
        : powerProfile === "power-saver" ? "󰌪" : "󰾅"
    readonly property color ppColor: powerProfile === "performance" ? "#f9e2af"
        : powerProfile === "power-saver" ? "#a6e3a1" : (theme.accent || "#89b4fa")
    readonly property bool ppActive: powerProfile !== "" && powerProfile !== "balanced"
    ...
    function cyclePowerProfile() {
        ...
        const target = order[(order.indexOf(cc.powerProfile) + 1) % order.length]
        Quickshell.execDetached([
            "busctl", "set-property",
            "net.hadess.PowerProfiles", "/net/hadess/PowerProfiles",
            "net.hadess.PowerProfiles", "ActiveProfile", "s", target
        ])
        cc.powerProfile = target        // optimistic; re-read to confirm
        ppRefreshTimer.restart()
    }
    ...
    Process {
        ...
        command: ["bash", "-c",
            ...
            "net.hadess.PowerProfiles ActiveProfile 2>/dev/null | awk '{gsub(/\"/,\"\",$2); print $2}'"]
        running: false
        stdout: SplitParser {
            onRead: data => cc.powerProfile = data.trim()
        }
    }
    ...
    Timer {
        ...
        onTriggered: cc.refreshPowerProfile()
    }
    ...
    // Pair (if needed), trust, then connect. Required to (re)add a device
    // that isn't paired yet — plain "connect" fails on unpaired devices.
    // A running agent (e.g. blueman-applet) shows the passkey dialog for
    // devices like keyboards that need one.
    function btPairConnect(mac) {
        if (!(hasBluetoothctl && hasBluetoothDevice))
            return
        const m = shellQuote(mac)
    ...
    Process {
        ...
        onExited: {
            const indexBySsid = {}
            const deduped = []
            for (const n of wifiScan.stdout.nets) {
                const at = indexBySsid[n.ssid]
                if (at === undefined) {
                    indexBySsid[n.ssid] = deduped.length
                    deduped.push(n)
                } else if (n.signal > deduped[at].signal) {
                    deduped[at] = n
                }
            }
            cc.wifiNetworks      = deduped
    ...
    Rectangle {
        ...
        // Smooth fade + grow-from-top-right corner, played both on open and
        // close (the window stays mapped until opacity reaches ~0).
        opacity: cc.showing ? 1 : 0
        Behavior on opacity {
        ...
        transform: Scale {
            ...
            origin.y: 0
            xScale: cc.showing ? 1 : 0.92
            yScale: cc.showing ? 1 : 0.92
            Behavior on xScale { NumberAnimation { duration: 190; easing.type: Easing.OutCubic } }
            Behavior on yScale { NumberAnimation { duration: 190; easing.type: Easing.OutCubic } }
        }
        ...
        Column {
            ...
            Rectangle {
                ...
                Row {
                    ...
                    Repeater {
                        model: [
                            { icon: "󰍃", title: "Logout",   message: "Log out of the current session?", hoverColor: theme.accent || "#89b4fa", cmd: ["niri", "msg", "action", "quit", "--skip-confirmation"] },
                            { icon: "󰜉", title: "Restart",  message: "Restart the system?",             hoverColor: "#fab387",                 cmd: ["systemctl", "reboot"] },
                            { icon: "󰐥", title: "Shutdown", message: "Power off the system?",           hoverColor: "#f38ba8",                 cmd: ["systemctl", "poweroff"] }
                        ]
                        delegate: Rectangle {
            ...
            // ── TOGGLES ROW 2 — Power profile (cycles on click) ──────────────
            Rectangle {
                width: parent.width; height: 52; radius: 12
                visible: cc.powerProfile !== ""
                color: cc.ppActive ? Qt.alpha(cc.ppColor, 0.15) : Qt.alpha(theme.dim || "#45475a", 0.35)
                border.color: cc.ppActive ? Qt.alpha(cc.ppColor, 0.4) : "transparent"
                border.width: 1
                Behavior on color { ColorAnimation { duration: 160 } }
                ...
                Row {
                    ...
                    Text {
                        ...
                        text: cc.ppIcon
                        color: cc.ppActive ? cc.ppColor : (theme.muted || "#585b70")
                        font.pixelSize: 22; font.family: "JetBrainsMono Nerd Font Propo"
                        Behavior on color { ColorAnimation { duration: 160 } }
                    }
                    Column {
                        anchors.verticalCenter: parent.verticalCenter
                        ...
                        Text {
                            text: "Power Mode"
                            color: cc.ppActive ? (theme.fg || "#cdd6f4") : (theme.muted || "#585b70")
                ...
                MouseArea {
                    anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                    onClicked: cc.cyclePowerProfile()
                }
            }
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor; hoverEnabled: true
                            onClicked: {
                                cc.btPairConnect(modelData.mac)
                                cc.showBtList = false
                            }
                        }
                    }
                    ...
                    Column {
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                readonly property bool showPwInput: cc.wifiPasswordOpen && cc.wifiPasswordSsid === modelData.ssid
                                width: wifiListCol.width
                                height: 38 + (showPwInput ? pwCol.implicitHeight + 12 : 0)
                                radius: 8
                                ...
                                onShowPwInputChanged: if (showPwInput) pwInput.forceActiveFocus()

                                RowLayout {
                                    anchors { left: parent.left; right: parent.right; top: parent.top; leftMargin: 10; rightMargin: 10 }
                                    height: 38
                                    spacing: 8
                                ...
                                Column {
                                    id: pwCol
                                    visible: wifiMgrItem.showPwInput
                                    anchors { left: parent.left; right: parent.right; top: parent.top; topMargin: 42; leftMargin: 10; rightMargin: 10 }
                                    spacing: 6
                                    ...
                                    Rectangle {
                                        ...
                                        height: 30
                                        radius: 8
                                        color: Qt.alpha(cc.theme.dim || "#45475a", 0.92)
                                        border.color: Qt.alpha(cc.theme.accent || "#89b4fa", 0.45)
                                        border.width: 1
                                        clip: true
